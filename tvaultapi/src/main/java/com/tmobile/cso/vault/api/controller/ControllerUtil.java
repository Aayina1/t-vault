...
public final class ControllerUtil {
	...
	/**
	 * Recursively reads the folders/secrets for a given path to get the secret
	 * count.
	 * 
	 * @param jsonstr
	 * @param token
	 ...
	 * @return
	 */
	public static SafeNode recursiveReadForCount(String jsonstr, String token, Response responseVO, String _path,
			String type) {
			...
			if (!lisresp.getResponse().contains("errors")) {
				try {
					...
					for (JsonNode node : folders) {
						...
						SafeNode sn;
						/* Recursively read the folders for the given folder/sub folders */
						sn = recursiveReadForCount(jsonstr, token, responseVO, path + "/" + node.asText(),
								TVaultConstants.FOLDER);
						sn.setId(path + "/" + node.asText());
						sn.setValue(path + "/" + node.asText());
						sn.setType(TVaultConstants.FOLDER);
						sn.setParentId(safeNode.getId());
				...
				log.error(JSONUtil.getJSON(ImmutableMap.<String, String>builder()
						...
						.put(LogMessage.MESSAGE,
								String.format("Unable to recursively read the given path [%s]", jsonstr))
						.put(LogMessage.APIURL, ThreadLocalContext.getCurrentMap().get(LogMessage.APIURL).toString())
						.build()));
				responseVO.setSuccess(false);
				responseVO.setHttpstatus(HttpStatus.INTERNAL_SERVER_ERROR);
			...
			if (!lisresp.getResponse().contains("errors")) {
				try {
					...
					log.error(JSONUtil.getJSON(ImmutableMap.<String, String>builder()
							...
							.put(LogMessage.MESSAGE,
									String.format("Unable to getFoldersAndSecrets [%s]", e.getMessage()))
							.put(LogMessage.APIURL,
									ThreadLocalContext.getCurrentMap().get(LogMessage.APIURL).toString())
							.build()));
					responseVO.setSuccess(false);
	...
	public static Response configureLDAPUser(String userName, String policies, String groups, String token) {
		log.error(JSONUtil.getJSON(ImmutableMap.<String, String>builder()
				...
				.put(LogMessage.MESSAGE,
						String.format("Trying configureLDAPUse with username [%s] policies [%s] and groups [%s] ",
		...
		try {
			...
			log.error(JSONUtil.getJSON(ImmutableMap.<String, String>builder()
					...
					.put(LogMessage.MESSAGE, String.format(
							"Unable to create ldapUserConfigJson [%s] with username [%s] policies [%s] and groups [%s] ",
							e.getMessage(), userName, policies, groups))
					.put(LogMessage.APIURL, ThreadLocalContext.getCurrentMap().get(LogMessage.APIURL).toString())
					.build()));
		}
	...
	public static Response configureUserpassUser(String userName, String policies, String token) {
		...
		try {
			...
			log.error(JSONUtil.getJSON(ImmutableMap.<String, String>builder()
					...
					.put(LogMessage.MESSAGE,
							String.format(
									"Unable to create userpassUserConfigJson [%s] with userName [%s] policies [%s] ",
									e.getMessage(), userName, policies))
					.put(LogMessage.APIURL, ThreadLocalContext.getCurrentMap().get(LogMessage.APIURL).toString())
					.build()));
	...
	public static Response configureLDAPGroup(String groupName, String policies, String token) {
		...
		try {
			...
			log.error(JSONUtil.getJSON(ImmutableMap.<String, String>builder()
					...
					.put(LogMessage.MESSAGE,
							String.format("Unable to create ldapConfigJson [%s] with groupName [%s] policies [%s] ",
									e.getMessage(), groupName, policies))
					.put(LogMessage.APIURL, ThreadLocalContext.getCurrentMap().get(LogMessage.APIURL).toString())
					.build()));
		}
	...
	public static Response configureAWSRole(String roleName, String policies, String token) {
		...
		try {
			...
			log.error(JSONUtil.getJSON(ImmutableMap.<String, String>builder()
					...
					.put(LogMessage.MESSAGE,
							String.format("Unable to create awsConfigJson [%s] with roleName [%s] policies [%s] ",
									e.getMessage(), roleName, policies))
					.put(LogMessage.APIURL, ThreadLocalContext.getCurrentMap().get(LogMessage.APIURL).toString())
					.build()));
		}
	...
	public static Response configureAWSIAMRole(String roleName, String policies, String token) {
		...
		try {
			...
			log.error(JSONUtil.getJSON(ImmutableMap.<String, String>builder()
					...
					.put(LogMessage.MESSAGE,
							String.format(
									"Unable to create awsConfigJson with message [%s] for roleName [%s] policies [%s] ",
									e.getMessage(), roleName, policies))
					.put(LogMessage.APIURL, ThreadLocalContext.getCurrentMap().get(LogMessage.APIURL).toString())
					.build()));
		}
		...
		if (HttpStatus.OK.equals(metadataResponse.getHttpstatus())) {
			try {
				...
				log.error(JSONUtil.getJSON(ImmutableMap.<String, String>builder()
						...
						.put(LogMessage.ACTION, UPDATEMETADATASTR)
						.put(LogMessage.MESSAGE, String.format(
								"Error creating _metadataMap for type [%s], name [%s], access [%s] and path [%s] message [%s]",
								_type, name, access, path, e.getMessage()))
						.put(LogMessage.APIURL, ThreadLocalContext.getCurrentMap().get(LogMessage.APIURL).toString())
						.build()));
			}
			...
			try {
				...
				log.error(JSONUtil.getJSON(ImmutableMap.<String, String>builder()
						...
						.put(LogMessage.ACTION, UPDATEMETADATASTR)
						.put(LogMessage.MESSAGE, String.format(
								"Error in creating metadataJson for type [%s], name [%s], access [%s] and path [%s] with message [%s]",
								_type, name, access, path, e.getMessage()))
						.put(LogMessage.APIURL, ThreadLocalContext.getCurrentMap().get(LogMessage.APIURL).toString())
						.build()));
			}
	...
	public static Response updateMetaDataOnConfigChanges(String name, String type, String currentPolicies,
			String latestPolicies, String token) {
		...
		for (String policy : _new) {
			...
			if (policyInfo.length == 3) {
				...
				switch (policyInfo[0]) {
				case "r":
					access = TVaultConstants.READ_POLICY;
					break;
				case "w":
					access = TVaultConstants.WRITE_POLICY;
					break;
				default:
					access = TVaultConstants.DENY_POLICY;
					break;
				}
	...
	private static Response getupdateMetaDataResponse(Map<String, String> sdbAccessMap, String type, String name,
			String token) {
		...
		if (HttpStatus.OK.equals(metadataResponse.getHttpstatus())) {
			try {
				...
				log.error(JSONUtil.getJSON(ImmutableMap.<String, String>builder()
						...
						.put(LogMessage.ACTION, UPDATEMETADATASTR)
						.put(LogMessage.MESSAGE, String.format(
								"Error creating _metadataMap for type service account update, name [%s], and path [%s] message [%s]",
								serviceAccount.getName(), _path, e.getMessage()))
						.put(LogMessage.APIURL, ThreadLocalContext.getCurrentMap().get(LogMessage.APIURL).toString())
						.build()));
			}
			...
			if (serviceAccount.getOwner() != null && !serviceAccount.getOwner().equals(TVaultConstants.EMPTY)
					&& !metadataMap.get("managedBy").equals(serviceAccount.getOwner())) {
		...
		if (HttpStatus.OK.equals(metadataResponse.getHttpstatus())) {
			try {
				...
				log.error(JSONUtil.getJSON(ImmutableMap.<String, String>builder()
						...
						.put(LogMessage.ACTION, UPDATEMETADATASTR)
						.put(LogMessage.MESSAGE,
								String.format("Error creating _metadataMap for type [%s] and path [%s] message [%s]",
										_type, path, e.getMessage()))
						.put(LogMessage.APIURL, ThreadLocalContext.getCurrentMap().get(LogMessage.APIURL).toString())
						.build()));
			}
			...
			if (StringUtils.isEmpty(initialPasswwordReset) || !initialPasswwordReset) {
				...
				try {
					...
					log.error(JSONUtil.getJSON(ImmutableMap.<String, String>builder()
							...
							.put(LogMessage.ACTION, UPDATEMETADATASTR)
							.put(LogMessage.MESSAGE, String.format(
									"Error in creating metadataJson for type [%s] and path [%s] with message [%s]",
									_type, path, e.getMessage()))
							.put(LogMessage.APIURL,
									ThreadLocalContext.getCurrentMap().get(LogMessage.APIURL).toString())
							.build()));
	...
	public static Map<String, Object> parseJson(String jsonString) {
		...
		try {
			...
			log.error(JSONUtil.getJSON(ImmutableMap.<String, String>builder()
					.put(LogMessage.USER, ThreadLocalContext.getCurrentMap().get(LogMessage.USER).toString())
					.put(LogMessage.ACTION, "parseJson").put(LogMessage.MESSAGE, "parseJson failed ")
					.put(LogMessage.APIURL, ThreadLocalContext.getCurrentMap().get(LogMessage.APIURL).toString())
					.build()));
		}
		return response;
	...
	public static String convetToJson(Map<String, Object> jsonMap) {
		...
		try {
			...
			log.error(JSONUtil.getJSON(ImmutableMap.<String, String>builder()
					.put(LogMessage.USER, ThreadLocalContext.getCurrentMap().get(LogMessage.USER).toString())
					.put(LogMessage.ACTION, "convetToJson").put(LogMessage.MESSAGE, "convetToJson failed ")
					.put(LogMessage.APIURL, ThreadLocalContext.getCurrentMap().get(LogMessage.APIURL).toString())
					.build()));
		}
	 ...
	 * @throws IOException
	 */
	public static String getPoliciesAsStringFromJson(ObjectMapper objMapper, String policyJson)
	 ...
	 * @throws IOException
	 */
	public static List<String> getPoliciesAsListFromJson(ObjectMapper objMapper, String policyJson)
			throws JsonProcessingException, IOException {
				...
				if (HttpStatus.OK.equals(userResponse.getHttpstatus())) {
					...
					try {
						...
						log.error(JSONUtil.getJSON(ImmutableMap.<String, String>builder()
								...
								.put(LogMessage.ACTION, UPDATEPOLICYONDELETE)
								.put(LogMessage.MESSAGE,
										String.format("updateUserPolicyAssociationOnSDBDelete failed [%s]",
												e.getMessage()))
								.put(LogMessage.APIURL,
										ThreadLocalContext.getCurrentMap().get(LogMessage.APIURL).toString())
								.build()));
			...
			for (String groupName : groups) {
				...
				if (HttpStatus.OK.equals(response.getHttpstatus())) {
					...
					if (TVaultConstants.LDAP.equals(vaultAuthMethod)) {
						...
						oidcUtil.updateGroupPolicies(token, groupName, policies, currentpolicies,
	...
	// Not using this method and decided to delete the role instead with the concept
	// that you cant have same role used by different safe.S
	public static void updateAwsRolePolicyAssociationOnSDBDelete(String sdb, Map<String, String> acessInfo,
			String token) {
		log.debug(JSONUtil.getJSON(ImmutableMap.<String, String>builder()
				.put(LogMessage.USER, ThreadLocalContext.getCurrentMap().get(LogMessage.USER).toString())
	...
	private static void updateAwsRolePolicyOnSDBDelete(String r_policy, String d_policy, String w_policy, String token,
			Map<String, String> acessInfo) {
			...
			if (HttpStatus.OK.equals(roleResponse.getHttpstatus())) {
				...
				try {
					...
					for (JsonNode policyNode : policiesArry) {
						currentpolicies = (currentpolicies.equals("")) ? currentpolicies + policyNode.asText()
								: currentpolicies + "," + policyNode.asText();
					}
				} catch (IOException e) {
					log.error(e);
					log.error(JSONUtil.getJSON(ImmutableMap.<String, String>builder()
							...
							.put(LogMessage.MESSAGE,
									String.format("Generation of currentpolicies failed for [%s]", e.getMessage()))
							.put(LogMessage.APIURL,
									ThreadLocalContext.getCurrentMap().get(LogMessage.APIURL).toString())
							.build()));
				}
	...
	public static void deleteAwsRoleOnSDBDelete(String sdb, Map<String, String> acessInfo, String token) {
		...
		if (acessInfo != null) {
			Set<String> roles = acessInfo.keySet();
			for (String role : roles) {
				Response response = reqProcessor.process("/auth/aws/roles/delete", "{\"role\":\"" + role + "\"}",
						token);
				if (response.getHttpstatus().equals(HttpStatus.NO_CONTENT)) {
					...
					log.debug(JSONUtil.getJSON(ImmutableMap.<String, String>builder()
							...
							.put(LogMessage.MESSAGE,
									String.format("%s, AWS Role is deleted as part of sdb delete. SDB path %s ", role,
											sdb))
							.put(LogMessage.APIURL,
									ThreadLocalContext.getCurrentMap().get(LogMessage.APIURL).toString())
							.build()));
					...
					log.error(JSONUtil.getJSON(ImmutableMap.<String, String>builder()
							...
							.put(LogMessage.ACTION, "deleteAwsRoleOnSDBDelete")
							.put(LogMessage.MESSAGE,
									String.format("%s, AWS Role is deletion failed. SDB path %s ", role, sdb))
							.put(LogMessage.APIURL,
									ThreadLocalContext.getCurrentMap().get(LogMessage.APIURL).toString())
							.build()));
				}
	...
	public static boolean isValidDataPath(String path) {
		...
		if (paths.length == 3) {
			...
			if (!(TVaultConstants.APPS.equals(safeType) || TVaultConstants.SHARED.equals(safeType)
					|| TVaultConstants.USERS.equals(safeType))) {
				return false;
			}
		} else {
			return false;
	...
	public static boolean areSDBInputsValid(Map<String, Object> requestParams) {
		...
		if (StringUtils.isEmpty(sdbName) || StringUtils.isEmpty(sdbOwner) || StringUtils.isEmpty(sdbDescription)
	...
	public static boolean areSDBInputsValid(Safe safe) {
		...
		if (StringUtils.isEmpty(sdbName) || StringUtils.isEmpty(sdbOwner) || StringUtils.isEmpty(sdbDescription)
				|| StringUtils.isEmpty(path) || StringUtils.isEmpty(appName)) {
			return false;
		}
		if (appName != null) {
	...
	public static boolean areSDBInputsValidForUpdate(Map<String, Object> requestParams) {
		...
		String path = (String) requestParams.get("path");
		if (StringUtils.isEmpty(sdbName) || StringUtils.isEmpty(sdbOwner) || StringUtils.isEmpty(sdbDescription)
				|| StringUtils.isEmpty(path) || StringUtils.isEmpty(sdbAppName)) {
			return false;
		}
		if (sdbAppName != null) {
	...
	public static boolean areSafeGroupInputsValid(Map<String, String> requestMap) {
		...
		if (ObjectUtils.isEmpty(requestMap.get("groupname")) || ObjectUtils.isEmpty(requestMap.get("path"))
	...
	public static boolean areAWSRoleInputsValid(Map<String, String> requestMap) {
		...
		if (ObjectUtils.isEmpty(requestMap.get("role")) || ObjectUtils.isEmpty(requestMap.get("path"))
				|| ObjectUtils.isEmpty(requestMap.get(ACCESSTR))) {
			return false;
		}
	...
	public static boolean areSafeUserInputsValid(SafeUser safeUser) {
		...
		if (ObjectUtils.isEmpty(safeUser.getUsername()) || ObjectUtils.isEmpty(safeUser.getAccess())
	...
	public static boolean areSafeUserInputsValid(Map<String, Object> requestMap) {
		...
		if (ObjectUtils.isEmpty(requestMap.get(USERNAMESTR)) || ObjectUtils.isEmpty(requestMap.get("path"))
				|| ObjectUtils.isEmpty(requestMap.get(ACCESSTR))) {
			return false;
		}
		String path = (String) requestMap.get("path");
	...
	public static boolean areSafeGroupInputsValid(SafeGroup safeGroup) {
		...
		if (ObjectUtils.isEmpty(safeGroup.getGroupname()) || ObjectUtils.isEmpty(safeGroup.getAccess())
	...
	public static boolean areAWSRoleInputsValid(AWSRole awsRole) {
		...
		if (ObjectUtils.isEmpty(awsRole.getRole()) || ObjectUtils.isEmpty(awsRole.getAccess())
	...
	private static String getSecretKey(String jsonString) {
		...
		try {
			Map<String, Object> requestParams = new ObjectMapper().readValue(jsonString,
					new TypeReference<Map<String, Object>>() {
					});
			LinkedHashMap<String, Object> map = (LinkedHashMap<String, Object>) requestParams.get("data");
	...
	public static boolean areAwsLoginInputsValid(AWSAuthType authType, AWSAuthLogin awsAuthLogin) {
		...
		if (AWSAuthType.EC2.equals(authType)) {
			...
			if (!StringUtils.isEmpty(awsAuthLogin.getIam_http_request_method())
	...
	public static boolean areAWSEC2RoleInputsValid(AWSLoginRole awsLoginRole) throws TVaultValidationException {
		...
		} else if (StringUtils.isEmpty(awsLoginRole.getAuth_type())
				|| !awsLoginRole.getAuth_type().equalsIgnoreCase("ec2")) {
	...
	public static boolean areAWSEC2RoleInputsValid(String jsonStr) throws TVaultValidationException {
		...
		if (StringUtils.isEmpty(map.get("role"))) {
			...
			throw new TVaultValidationException("auth_type is required and it should be ec2.");
		} else if (!StringUtils.isEmpty(map.get("bound_account_id")) || !StringUtils.isEmpty(map.get("bound_ami_id"))
				|| !StringUtils.isEmpty(map.get("bound_iam_instance_profile_arn"))
				|| !StringUtils.isEmpty(map.get("bound_iam_role_arn")) || !StringUtils.isEmpty(map.get("bound_region"))
				|| !StringUtils.isEmpty(map.get("bound_subnet_id")) || !StringUtils.isEmpty(map.get("bound_vpc_id"))) {
			return true;
		}
		throw new TVaultValidationException("At least one bound parameter should be specified.");
	}
	...
	public static boolean areAWSIAMRoleInputsValid(AWSIAMRole awsiamRole) throws TVaultValidationException {
		...
		if (StringUtils.isEmpty(awsiamRole.getRole())) {
			throw new TVaultValidationException(ROLESTR);
		} else if (StringUtils.isEmpty(awsiamRole.getAuth_type())
				|| !awsiamRole.getAuth_type().equalsIgnoreCase("iam")) {
		...
		if (response.getHttpstatus().equals(HttpStatus.OK)) {
			try {
				...
				log.error(JSONUtil.getJSON(ImmutableMap.<String, String>builder()
						...
						.put(LogMessage.MESSAGE,
								String.format("Unable to get list of safes due to [%s] ", e.getMessage()))
						.put(LogMessage.APIURL, ThreadLocalContext.getCurrentMap().get(LogMessage.APIURL).toString())
						.build()));
			}
		}
	...
	public static int getCountOfSafesForGivenSafeName(String safeName, String token) {
		...
		for (Map.Entry<String, List<String>> entry : allExistingSafeNames.entrySet()) {
			...
			for (String existingSafeName : existingSafeNames) {
				// Note: SafeName is duplicate if it is found in any type (Shared/User/Apps).
				// Hence no need to compare by prefixing with SafeType
	...
	public static String generateSafePath(String safeName, String safeType) {
		...
		switch (safeType) {
		case TVaultConstants.USERS:
		case "User Safe":
			safePath = "users/" + safeName;
			break;
		case TVaultConstants.SHARED:
		case "Shared Safe":
			safePath = "shared/" + safeName;
			break;
		case TVaultConstants.APPS:
	...
	public static boolean createMetadata(String metadataJson, String token) {
		...
		if (response.getHttpstatus().equals(HttpStatus.NO_CONTENT)) {
			...
			log.debug(JSONUtil.getJSON(ImmutableMap.<String, String>builder()
					.put(LogMessage.USER, ThreadLocalContext.getCurrentMap().get(LogMessage.USER).toString())
					.put(LogMessage.ACTION, CREATEMETADATASTR).put(LogMessage.MESSAGE, "Metadata created successfully ")
			...
			log.error(JSONUtil.getJSON(ImmutableMap.<String, String>builder()
					.put(LogMessage.USER, ThreadLocalContext.getCurrentMap().get(LogMessage.USER).toString())
					.put(LogMessage.ACTION, CREATEMETADATASTR).put(LogMessage.MESSAGE, "Failed to create metadata")
					.put(LogMessage.STATUS, response.getHttpstatus().toString())
					.put(LogMessage.APIURL, ThreadLocalContext.getCurrentMap().get(LogMessage.APIURL).toString())
					.build()));
		}
	...
	public static Response canDeleteRole(String roleName, String token, UserDetails userDetails, String metadataPath) {
		...
		response.setResponse(
				"Either role doesn't exist or you don't have enough permission to remove this role from Safe");
		response.setSuccess(false);
		return response;
	}
	...
	public static boolean validateInputs(String certificateName, String sslCertType) {
		...
		if (certificateName.contains(" ") || (!certificateName.endsWith(EMAILSTR)) || (certificateName.contains(".-"))
				|| (certificateName.contains("-.")) || (!sslCertType.matches(INTERNALEXTERNAL))) {
			return false;
		}
	 ...
	 * @throws JsonProcessingException
	 */
	public static boolean updateMetaDataOnPath(String path, Map<String, String> params, String token)
			throws JsonProcessingException {
	...
	public static boolean arecertificateGroupInputsValid(CertificateGroup certificateGroup) {
		...
		if (ObjectUtils.isEmpty(certificateGroup.getGroupname()) || ObjectUtils.isEmpty(certificateGroup.getAccess())
	...
	public static boolean canAddCertPermission(String path, String certificateName, String token) {
		...
		if (count != 0) {
			...
			// There are no certificates or more than one and hence permission can't be
			// added
			return false;
		}
	}
		...
		if (response.getHttpstatus().equals(HttpStatus.OK)) {
			try {
				...
				log.error(JSONUtil.getJSON(ImmutableMap.<String, String>builder()
						...
						.put(LogMessage.MESSAGE,
								String.format("Unable to get list of certificate due to [%s] ", e.getMessage()))
						.put(LogMessage.APIURL, ThreadLocalContext.getCurrentMap().get(LogMessage.APIURL).toString())
						.build()));
			}
		}
	...
	public static Response updateSslMetaDataOnConfigChanges(String name, String type, String currentPolicies,
			String latestPolicies, String token) {
	...
	public static Map<String, Object> filterDuplicateSvcaccPermissions(Map<String, Object> access) {
		if (!MapUtils.isEmpty(access)) {
			List<Map<String, String>> svcaccPermissions = (List<Map<String, String>>) access
					.get(TVaultConstants.SVC_ACC_PATH_PREFIX);
